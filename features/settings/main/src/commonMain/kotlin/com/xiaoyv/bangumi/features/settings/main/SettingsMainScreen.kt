    ...
    Column(modifier = Modifier.padding(vertical = 24.dp)) {
        ...
        SettingContainer(label = { Text(text = stringResource(Res.string.pixiv_account_section)) }) {
            SettingItem(
                ...
                supportingContent = {
                    Text(
                        text = if (pixivLoggedIn) {
                            val account = pixivUser?.account?.takeIf { it.isNotBlank() }
                                ?: pixivUser?.name
                            stringResource(Res.string.pixiv_manage_login_logged_in, account.orEmpty())
                        } else {
                            stringResource(Res.string.pixiv_account_not_login)
                        }
                    )
                },
